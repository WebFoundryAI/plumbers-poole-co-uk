...
const CUSTOMER_REVIEWS = [
  ...
  },
  {
    ...
    reviewRating: 5,
    reviewBody: "Used Plumbers Poole for a CCTV survey after repeated drainage issues. They identified the root cause and provided a clear report with options. Very thorough and reasonably priced."
  },
  {
...
export function getBaseBusinessSchema() {
  return {
    ...
    "description": `Professional plumbing, drain unblocking, CCTV surveys, and emergency drainage services across Poole and the surrounding Dorset area. Fast response, no call-out fee, 24/7 availability.`,
    "url": siteUrl,
    ...
    // TODO: Add real aggregateRating from Google Business Profile when available
    // "aggregateRating": {
...
export function getLocationPageSchema(location: Location) {
  ...
  const locationSchema = {
    ...
    "areaServed": {
      ...
      "containedInPlace": {
        "@type": "AdministrativeArea",
        "name": location.countyOrRegion || "Dorset"
      }
    },
    "geo": {
      "@type": "GeoCoordinates",
...
export function getLocationServicePageSchema(location: Location, service: Service) {
  ...
  const serviceSchema = {
    ...
    "description": `Professional ${service.name.toLowerCase()} services in ${location.name}, ${location.countyOrRegion || 'Dorset'}. Fast response, no call-out fee, 24/7 availability.`,
...
export function getWebSiteSchema() {
  return {
    ...
    "url": siteUrl,
    "name": BRAND.brandName,
    "description": `Professional plumbing, drain unblocking, CCTV surveys, and emergency drainage services across Poole and the surrounding Dorset area.`,
    "publisher": {
      "@id": `${siteUrl}/#organization`
    },
    "inLanguage": "en-GB",
