...
function generateSitemapUrls(): SitemapUrl[] {
  ...
  for (const [i, location] of INDEXED_LOCATIONS.entries()) {
    // Main location page (Poole gets higher priority)
    const isPoole = location.slug === 'poole';
    urls.push({
      loc: `/locations/${location.slug}/`,
      priority: isPoole ? 0.9 : 0.8,
      changefreq: 'weekly',
      lastmod: getStaggeredDate(isPoole ? 1 : 2 + i),
    });
